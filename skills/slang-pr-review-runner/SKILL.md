...
| `scripts/devin-fetch.sh` | Reviewer B | Drives `agent-browser` to load `app.devin.ai/review/...`, polls for analysis completion, captures commit-status freshness, expands the Bugs / Flags / Informational panels, and extracts the AI analysis + bug list + flag list + informational-finding list to `devin-flags.md`. Default timeout 30 minutes. Exits 2 on auth-wall, 3 on timeout, 4 on browser-launch-failure (transient — stale Chrome profile lock; the script clears it and retries once first) — workflow treats 2/3/4 as best-effort skip. |
...
| `scripts/post-back.sh` | Post-back (orchestration wrapper) | Convenience wrapper the workflow calls when it has decided to post: runs `cleanup.sh` then `post-review.sh` in sequence. One invocation, one exit code. Use this from the workflow rather than calling the two scripts manually. |
...
- **Devin scrape is brittle.** `devin-fetch.sh` keeps selectors minimal (heading text + Bugs/Flags buttons + commit-status popover via `aria-label`). The 2026 UI split the single "N Flags" toggle into separate Bugs and Flags buttons and added a third **Informational** finding category whose rows live only as button DOM nodes (not in `innerText`); the script clicks all three and harvests the Informational rows directly. When expanding it never clicks a finding's `Post to GitHub` sibling (that would post to the PR). DOM changes upstream will break it; the script fails gracefully and the workflow treats Reviewer B as best-effort.
- **Chrome-launch failures are transient, not deterministic.** Chrome launches fine in the container without a dbus session bus. A `Failed to launch Chrome` / `without writing DevToolsActivePort` / `SingletonLock` error means a stale profile under `/tmp/agent-browser-*` from a prior crash. `devin-fetch.sh` closes the daemon, clears the profile dirs, and retries `open` once; a failure that survives the retry exits **4** (transient, retry later). Do not describe it as a "deterministic environment failure" — re-running the review usually succeeds.
- **DONE detection tolerates a lingering "in progress" banner.** Devin's "PR analysis in progress" text can flicker back on a fully-rendered page. The done-check only lets it veto when the positive signals (AI-analysis heading + a Bugs/Flags/Checks summary) are absent, and requires the done state to hold across two consecutive polls — so a transient banner no longer causes a false 30-minute timeout, while a genuine re-analysis (new head commit) still keeps the poll waiting.
